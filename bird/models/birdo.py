...
def Birdo(nb_classes, input_shape):
    """ Instantiate a Birdo architecture
    ...
    x = Convolution2D(64, 5, 5, subsample=(1, 2), init="he_normal", border_mode="same", name='conv_1')(img_input)
    x = BatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    x = Activation('relu')(x)

    # max pooling (2x2 kernels, stride size 2x2)
    x = MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(x)
    shortcut = MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(x)
    shortcut = MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(shortcut)
    shortcut = Convolution2D(256, 1, 1, subsample=(1, 1), init="he_normal",
                             border_mode="same", name="conv_shortcut")(shortcut)

    # conv (64 5x5 kernels, stride size 1x1)
    x = Convolution2D(64, 5, 5, subsample=(1, 1), init="he_normal", border_mode="same", name='conv2')(x)
    ...
    x = Convolution2D(128, 5, 5, subsample=(1, 1), init="he_normal", border_mode="same", name='conv3')(x)
    ...
    x = Convolution2D(256, 5, 5, subsample=(1, 1), init="he_normal", border_mode="same", name='conv4')(x)
    ...
    x = Convolution2D(256, 3, 3, subsample=(1, 1), init="he_normal", border_mode="same", name='conv5')(x)
